...
# A1 W4 S1 E1 X8 C3 BB-no value

#best play => (62, (3, 2), (1, 0), 'BaCKBENCH') includes a lower case letter
...
LOWER_LETTERS = map(str.lowser, LETTERS)
...
def add_suffixes(hand, pre, start, row, results, anchored=True):
    ...
    i = start + len(pre)
    PRE = pre.upper() #allow lowercase letters in prefix
    ...
    if PRE in PREFIXES:
        ...
        if is_letter(sq):
            ...
            if '_' b in hand: #allow blanks
                for C in possibilities:
                    add_suffixes(hand.replace('_', '', 1), pre+C.lower(), star, row, results)
    return results
...
def find_prefixes(hand, pre='', results=None):
    ...
    PRE = pre.upper() #uppercase it before test!
    if PRE in WORDS or PRE in PREFIXES: results.add(pre)
    if PRE in PREFIXES:
        for L in hand:
            if L == '_': #allow blank
                for C in LOWER_LETTERS: #blank tiles as low case
