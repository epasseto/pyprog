...
def old_dictiograph(graph): # create a dictionnaire for the graph
    dictiog = {}
    while len(graph)>0:
        link = graph.pop()
        a = link[0]
        ...
        if a in dictiog:
            ...
            if a < b:
                ...
                c.append([b,1]) #is a reverse path - not to visit twice!
                dictiog[a] = c
        else:
            if a < b:
                dictiog[a] = [[b,0]]
...
def tweak_entry(dic, neighbors, node):
    ...
    while node < len(set):
        if node in dic:
            ...
            while True:
                print node, dic
                if len(neighbors) > 0:
                    answer = tweak_entry(dic, neighbors, node)
                    if answer == None:
